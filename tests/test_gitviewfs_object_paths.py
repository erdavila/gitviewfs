...
from gitviewfs_objects import get_gitviewfs_object, Directory, HeadSymLink,\
	...
	OldCommitPersonDateFile, CommitPersonDir, OldCommitPersonNameFile,\
	OldCommitPersonEmailFile, CommitTreeSymLink, CommitParentsDir,\
	CommitParentSymLink, TreesDir, TreeDir, TreeDirItem, BlobsDir, BlobFile
...
class TestPaths(unittest.TestCase):
	...
	def test_BranchesDir(self):
		self.assertPathClassAndName(paths.BRANCHES_DIR, Directory, 'branches')
	...
	def test_ObjectsDir(self):
		self.assertPathClassAndName(paths.OBJECTS_DIR, Directory, 'objects')
	
	def test_CommitsDir(self):
		self.assertPathClassAndName(paths.COMMITS_DIR, Directory, 'commits')
	
	def test_CommitDir(self):
		self.assertPathClassAndName(paths.COMMIT_DIR, Directory, paths.SAMPLE_HASH)
	...
	def test_CommitAuthorNameFile(self):
		self.assertPathClassAndName(paths.COMMIT_AUTHOR_NAME_FILE, CommitPersonNameFile, 'name')
	
	def test_CommitAuthorEmailFile(self):
		self.assertPathClassAndName(paths.COMMIT_AUTHOR_EMAIL_FILE, CommitPersonEmailFile, 'email')
	
	def test_CommitAuthorDateFile(self):
		self.assertPathClassAndName(paths.COMMIT_AUTHOR_DATE_FILE, OldCommitPersonDateFile)
